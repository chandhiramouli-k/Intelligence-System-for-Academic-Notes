...
An intelligent RAG (Retrieval-Augmented Generation) system built to help students chat with their academic notes. This project uses **Groq** for lightning-fast inference and **HuggingFace** for local embeddings.

## 📂 Project Structure
```text
ACADEMIC_NOTES_AI/
├── app/
│   ├── chunking.py      # Logic for loading and splitting notes
│   ├── embed_store.py   # FAISS vector database management
│   ├── main.py          # Main entry point and chat loop
│   ├── prompt.py        # Custom system prompt templates
│   ├── qa_engine.py     # RAG chain configuration
│   └── search.py        # Similarity search logic
├── data/
│   └── notes.txt        # Your academic content (Input file)
├── embeddings/          # Cache for local embedding models
